...
from crypto import Prf, Cipher, DiffieHellman, Integrity, RsaPrivateKey, RsaPublicKey, ECDH
...
class TestCrypto(unittest.TestCase):
    def test_dh(self):
        dh1 = DiffieHellman.from_group(14)
        dh2 = DiffieHellman.from_group(14)
        dh1.compute_secret(dh2.public_key)
        dh2.compute_secret(dh1.public_key)
        self.assertEqual(dh1.shared_secret, dh2.shared_secret)

    def test_dh_invalid_group(self):
        with self.assertRaises(KeyError):
            dh1 = DiffieHellman.from_group(140)

    def test_ecdh(self):
        dh1 = DiffieHellman.from_group(19)
        dh2 = DiffieHellman.from_group(19)
        dh1.compute_secret(dh2.public_key)
        dh2.compute_secret(dh1.public_key)
        self.assertEqual(dh1.shared_secret, dh2.shared_secret)

    def test_ecdh_invalid_len(self):
        dh1 = DiffieHellman.from_group(19)
        dh2 = DiffieHellman.from_group(19)
        dh1.compute_secret(dh2.public_key)
        with self.assertRaises(ValueError):
            dh2.compute_secret(dh1.public_key[:10])
    ...
    def test_prf(self):
        ...
        self.assertEqual(prfplus, b'\xdbeb\x11F\xbf\xf2Y\xadC\xbd\xba\xc4\xe9\xdd\xf2\x10\x82\r\xd5\x85\xa6h2l\xcf\x98'
    ...
    def test_integrity(self):
        ...
        self.assertEqual(checksum, b'\x0e\xb2\x8a\xa0N\x14\x0b$\x9a\x8c/\x9d<\x83\xd2\xf8\x94\x12\x1a\xbc\xd4b~'
